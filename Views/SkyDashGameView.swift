...
class SkyFallHighScoreManager {
    static let shared = SkyFallHighScoreManager()
    
    private let highScoreKey = "skyFallHighScore"
...
// MARK: - Motion Manager
class SkyFallMotionManager: ObservableObject {
...
// MARK: - Sky Fall Game View
struct SkyFallGameView: View {
    ...
    // Motion
    @StateObject private var motionManager = SkyFallMotionManager()
    
    // Game state
    @State private var gameState: SkyFallState = .ready
    @State private var score: Int = 0
    @State private var highScore: Int = SkyFallHighScoreManager.shared.highScore
    ...
    private func startGame() {
        ...
        guard userSettings.useGameCredit(for: .skyDash) else {
    ...
    private func endGame() {
        ...
        if distance > highScore {
            highScore = distance
            SkyFallHighScoreManager.shared.highScore = distance
            isNewHighScore = true
            HapticFeedback.success.trigger()
        } else {
...
// MARK: - Preview
#Preview {
    SkyFallGameView(onComplete: { _ in })
